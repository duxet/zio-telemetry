...
import zio.telemetry.opentelemetry.SpanPropagation.{ extractContext, injectContext }
import zio._
...
object Tracing {
  ...
  def spanFrom[C, R, E, A](
    ...
    for {
      extractedSpan <- extractContext(propagator, carrier, getter)
  ...
  def managed(tracer: Tracer): URManaged[Clock, Service] = {
    ...
    val tracing: URIO[Clock, Service] =
      for {
        ...
        defaultSpan <- FiberRef.make[Context](Context.root())
